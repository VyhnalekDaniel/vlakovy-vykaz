...
# Sem si později doplníš svoje přednastavené poznámky k ZOB.
# Uživatel jich bude moct zaškrtnout víc najednou.
...
def cislo_vlaku_zmeneno(stare_info, nove_info):
    stare = str(stare_info.get("vlak", "")).strip()
    nove = str(nove_info.get("vlak", "")).strip()
    return bool(stare and nove and stare != nove)


def priprav_zmenu_vlaku(nove_info, navrat):
    session["cekajici_info_vlaku"] = nove_info
    session["navrat_po_zmene_vlaku"] = navrat
    session.modified = True
...
def potvrdit_zmenu_vlaku():
    ...
    nove_info = session.get("cekajici_info_vlaku")
    if not nove_info:
        return redirect(url_for("vlakova_dokumentace"))

    if request.method == "POST":
        ponechat = request.form.get("ponechat_vozidla") == "ANO"
        info_vlaku.update(nove_info)
        if not ponechat:
            seznam_vozidel = []
            zadana_cisla = []
            flash("Číslo vlaku bylo změněno a výkaz vozidel byl vymazán.", "success")
        else:
            flash("Číslo vlaku bylo změněno a vozidla ve výkazu zůstala zachována.", "success")
        aktualizuj_brzdici_procenta(info_vlaku, seznam_vozidel)
        save_state(info_vlaku, seznam_vozidel, zadana_cisla)
        session.pop("cekajici_info_vlaku", None)
        navrat = session.pop("navrat_po_zmene_vlaku", "vykaz_vozidel")
        session.modified = True
        return redirect(url_for(navrat))

    return render_template(
        "potvrdit_zmenu_vlaku.html",
        stare_info=info_vlaku,
        nove_info=nove_info,
        pocet=len(seznam_vozidel),
    )
...
def vlak_z_jr():
    ...
    if request.method == "POST":
        ...
        if request.form.get("akce") == "pouzit" and nalezeny_vlak:
            nove_info = deepcopy(info_vlaku)
            nove_info.update(nalezeny_vlak)
            if seznam_vozidel and cislo_vlaku_zmeneno(info_vlaku, nove_info):
                priprav_zmenu_vlaku(nove_info, "vykaz_vozidel")
...
def libovolny_vlak():
    ...
    if request.method == "POST":
        nove_info = deepcopy(info_vlaku)
        uloz_data_vlaku_z_formulare(nove_info, request.form)
        nove_info["potrebna_procenta"] = to_float(request.form.get("potrebna_procenta"), info_vlaku.get("potrebna_procenta", 0))
        if seznam_vozidel and cislo_vlaku_zmeneno(info_vlaku, nove_info):
            priprav_zmenu_vlaku(nove_info, "vykaz_vozidel")
            return redirect(url_for("potvrdit_zmenu_vlaku"))
        info_vlaku.update(nove_info)
...
@app.route("/vozidlo/<int:pozice>/posun/<smer>", methods=["POST"])
def posun_vozidlo(pozice, smer):
    info_vlaku, seznam_vozidel, zadana_cisla = get_state()
    index = pozice - 1
    if index < 0 or index >= len(seznam_vozidel):
        flash("Neplatná pozice vozidla.", "error")
        return redirect(url_for("vykaz_vozidel"))
    ...
    if smer == "nahoru":
        ...
        flash("Neplatný směr posunu.", "error")
        return redirect(url_for("vykaz_vozidel"))

    if novy_index < 0 or novy_index >= len(seznam_vozidel):
        return redirect(url_for("vykaz_vozidel"))

    seznam_vozidel[index], seznam_vozidel[novy_index] = seznam_vozidel[novy_index], seznam_vozidel[index]
    aktualizuj_brzdici_procenta(info_vlaku, seznam_vozidel)
    save_state(info_vlaku, seznam_vozidel, zadana_cisla)
    flash("Pozice vozidla byla změněna.", "success")
    return redirect(url_for("vykaz_vozidel"))


@app.route("/vozidlo/<int:pozice>/presun", methods=["POST"])
def presun_vozidlo(pozice):
    info_vlaku, seznam_vozidel, zadana_cisla = get_state()
    index = pozice - 1
    if index < 0 or index >= len(seznam_vozidel):
        flash("Neplatná pozice vozidla.", "error")
    ...
    try:
        nova_pozice = int(request.form.get("nova_pozice", 0))
    except ValueError:
        flash("Nová pozice musí být číslo.", "error")
        return redirect(url_for("vykaz_vozidel"))

    if nova_pozice < 1 or nova_pozice > len(seznam_vozidel):
        flash("Nová pozice je mimo rozsah seznamu.", "error")
        return redirect(url_for("vykaz_vozidel"))

    vozidlo = seznam_vozidel.pop(index)
    seznam_vozidel.insert(nova_pozice - 1, vozidlo)
    aktualizuj_brzdici_procenta(info_vlaku, seznam_vozidel)
    save_state(info_vlaku, seznam_vozidel, zadana_cisla)
    flash("Vozidlo bylo přesunuto na novou pozici.", "success")
    return redirect(url_for("vykaz_vozidel"))
...
def zob():
    info_vlaku, seznam_vozidel, zadana_cisla = get_state()
    if request.method == "POST":
        # Nejdřív vždy uložíme údaje ze ZOB. Díky tomu se při přechodu do JZB/ÚZB
        # neztratí potřebná brzdicí procenta, doprovod ani poznámky.
        info_vlaku["potrebna_procenta"] = to_float(
            request.form.get("potrebna_procenta"),
            info_vlaku.get("potrebna_procenta", 0),
        )
        ...
        info_vlaku["poznamky_zob_vyber"] = request.form.getlist("poznamky_zob_vyber")
        aktualizuj_brzdici_procenta(info_vlaku, seznam_vozidel)
        save_state(info_vlaku, seznam_vozidel, zadana_cisla)

        akce = request.form.get("akce", "rekap")
        if akce == "jzb":
            flash("ZOB byla průběžně uložena. Nyní vyplň JZB.", "success")
            return redirect(url_for("detail_zkousky", typ="JZB"))
        if akce == "uzb":
            flash("ZOB byla průběžně uložena. Nyní vyplň ÚZB.", "success")
            return redirect(url_for("detail_zkousky", typ="UZB"))

        flash("Zpráva o brzdění byla uložena.", "success")
        return redirect(url_for("rekapitulace"))
    data = vytvor_vykaz(info_vlaku, seznam_vozidel) if seznam_vozidel else None
    return render_template("zob.html", info=info_vlaku, rezimy=REZIMY, data=data, poznamky_moznosti=POZNAMKY_ZOB_MOZNOSTI)
...
def detail_zkousky(typ):
    ...
    if request.method == "POST":
        ...
        kym = request.form.get("kym", "")
        if kym == "Jiný ŽDP":
            jiny = request.form.get("kym_jiny", "").strip()
            if jiny:
                kym = f"Jiný ŽDP - {jiny}"
        info_vlaku[f"{prefix}_kym"] = kym
        # Poznámky u JZB/ÚZB už nezadáváme, zůstávají jen poznámky k ZOB.
        info_vlaku[f"{prefix}_poznamka"] = ""
        ...
        return redirect(url_for("zob"))
    return render_template("zkouska_detail.html", typ=typ, prefix=prefix, info=info_vlaku, vykonal_moznosti=VYKONAL_MOZNOSTI)
